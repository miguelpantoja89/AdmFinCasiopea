    ...
    if ($_FILES["file"]["size"] > 0) {
        ...
        while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            if($error!=""){
                ...
                insertarPropietarioCSV($conexion, $column);
...
function insertarPropietarioCSV($conexion, $column){
    try{
        $stmn = $conexion -> prepare("INSERT INTO Propietarios (NOMBREAP, DNI, TELEFONO, EMAIL) VALUES (:Nombre, :dni, :telefono, :email)");
        $stmn -> bindParam(':Nombre', $column[0]);
        $stmn -> bindParam(':dni', $column[1]);
        $stmn -> bindParam(':telefono', $column[2]);
        $stmn -> bindParam(':email', $column[3]);
        $stmn -> execute();
